...
export class ApiService {
  ...
  constructor(
    ...
    this.apiEndpoint = endpoint.OLD_URL;
  ...
  public setNewEndpoint(){
    this.apiEndpoint = this.endpoint.NEW_URL;
  }

  public setOldEndpoint(){
    this.apiEndpoint = this.endpoint.OLD_URL;
  }
  ...
  public getMeanHealth(spaceMarines: Array<SpaceMarine>){
    ...
    return  this
      .http
      .get(this.apiEndpoint + this.apiMeanHealth + "?" + requestString.slice(1, requestString.length),
        {responseType: 'text'})
      .pipe(
        map((res) => {
          let xmlToJson = converter.xml2json(res, {compact: true, spaces: 2});
